<x-admin.layout>
    <x-slot name="title">Edit Book: {{ $book->book_name }}</x-slot>
    
    <div class="flex flex-col gap-1 lg:gap-2">
        <h1 class="font-semibold md:text-lg">Edit Book</h1>
        <div class="flex items-center gap-1">
            ...
            <p class="text-xs text-gray-400 md:text-sm">/</p>
            <p class="text-xs text-gray-400 md:text-sm">Edit</p>
        </div>
        ...
        <div class="bg-white shadow-md rounded-lg p-6 my-4">
            ...
            <form action="{{ route('admin.book.update', $book->book_id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="book_isbn" id="book_isbn" placeholder="Book ISBN (e.g., 978-3-16-148410-0)" 
                           ...
                           value="{{ old('book_isbn', $book->book_isbn) }}">
                    @error('book_isbn')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="book_description" id="book_description" placeholder="Book Description" rows="3"
                              ...
                              required>{{ old('book_description', $book->book_description) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="book_img" id="book_img" placeholder="e.g. https://placehold.co/150x200" 
                           ...
                           value="{{ old('book_img', $book->book_img) }}">
                    @error('book_img')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="book_category_id" id="book_category_id" 
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->category_id }}" {{ old('book_category_id', $book->book_category_id) == $category->category_id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="book_publisher_id" id="book_publisher_id" 
                        ...
                        @foreach($publishers as $publisher)
                            <option value="{{ $publisher->publisher_id }}" {{ old('book_publisher_id', $book->book_publisher_id) == $publisher->publisher_id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="book_author_id" id="book_author_id" 
                        ...
                        @foreach($authors as $author)
                            <option value="{{ $author->author_id }}" {{ old('book_author_id', $book->book_author_id) == $author->author_id ? 'selected' : '' }}>
                                {{ $author->author_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="book_shelf_id" id="book_shelf_id" 
                        ...
                        @foreach($shelves as $shelf)
                            <option value="{{ $shelf->shelf_id }}" {{ old('book_shelf_id', $book->book_shelf_id) == $shelf->shelf_id ? 'selected' : '' }}>
                                {{ $shelf->shelf_name }}
                            </option>
                        @endforeach
                    </select>
