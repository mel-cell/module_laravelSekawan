<div>
    <div class="overflow-x-auto bg-white rounded-lg shadow-md">
        <div class="mb-4 p-4">
            <input wire:model.live="search" type="search" placeholder="Search categories..." class="w-full px-4 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-blue-300">
        </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($categories as $index => $category)
                    <tr class="text-gray-700">
                        <td class="px-4 py-3 text-sm">{{ $index + $categories->firstItem() }}</td>
                        <td class="px-4 py-3 text-sm">{{ $category->category_name }}</td>
                        <td class="px-4 py-3 text-sm">{{ \Illuminate\Support\Str::limit($category->category_description, 35) }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end space-x-2">
                                <button onclick="editCategory('{{ $category->category_id }}', '{{ $category->category_name }}', '{{ $category->category_description }}')"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-3 text-center text-sm text-gray-500">No categories found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <script>
        ...
        function editCategory(id, name, description) {
            ...
            // Pastikan URL sesuai dengan route Anda, misalnya /admin/category/${id}
